...
with col2:
    if uploaded_file:
        if uploaded_file.size > 5 * 1024 * 1024:
            ...
            st.image(image_data, caption="🖼️ Uploaded X-Ray Image", use_container_width=True)
...
if user_message:
    if st.session_state['image_uploaded'] and st.session_state['report_text']:
        # 1) Build a single prompt that includes the previous report
        context_prompt = (
            "You are a helpful medical assistant.  \n"
            "Here is the X‑ray analysis report:\n"
            f"{st.session_state['report_text']}\n\n"
            "Based on that report, answer the user’s question below clearly and in plain language:\n"
            f"{user_message}"
        )

        with st.spinner("🧠 AI is responding..."):
            chat_session = model.start_chat()
            # pass the combined prompt as a single message
            response = chat_session.send_message([context_prompt])

        st.write(f"**AI Response:** {response.text}")
        # keep track of conversation if you want
        st.session_state['history'].append({"role": "user",    "content": user_message})
        st.session_state['history'].append({"role": "assistant","content": response.text})
    else:
        st.warning("⚠️ Please analyze an X‑ray image first before chatting.")
